...
# All rights reserved.
#
from moenum.match import Case, RangeCase
...
fn assert_exhaustive_or_warn(cases: List[Case], ranges: List[RangeCase], u_lo: UInt64, u_hi: UInt64) -> Bool:
    ...
    if len(holes) == 0:
        return True
    print(String("[moenum] Non-exhaustive match; missing tags: ") + String(holes))
    return False

fn assert_no_duplicates_or_warn(cases: List[Case]) -> Bool:
    var dups = find_duplicate_cases(cases)
    if len(dups) == 0: return True
    print(String("[moenum] Duplicate exact cases: ") + String(dups))
    return False
